    ...
    if intervals == MAX_INTERVALS:
        ...
        try:
            if (hour == 10) and (min < 10):
                b.send_msg(choice(b.phrases_gm), onlineIDs, start + choice(b.pics))
                print_time(hour, min, sec)
            elif (hour == 23) and (time.minute < 10):
                b.send_msg(choice(b.phrases_gn) + ' ' + choice(smiles_gn), onlineIDs, start + choice(b.pics_gn))
                print_time(hour, min, sec)
            elif (hour >= 10 and hour <= 22):
                b.send_msg(choice(b.phrases) + ' ' + choice(smiles), onlineIDs, start + choice(b.pics))
                print_time(hour, min, sec)
